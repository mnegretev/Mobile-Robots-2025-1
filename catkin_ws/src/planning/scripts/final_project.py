...
def parse_command(cmd):
    ...
    loc = [1.25,-1.73] if "TABLE" in cmd else [1.49, 10.5]
    return obj, loc
...
#
#Initialize ROS node, subscribers and publishers for robot control and communication
#
def main():
    ...
    goal_reached = False
  #
  #Main loop for state machine: processes task based on false speech, moves the robot and controls the arm
  #
    while not rospy.is_shutdown():
        ...
        if current_state == "SM_INIT":
            ...
            if goal_x == 1.49 and goal_y == 10.5:
                loc_name = "TABLE"
            elif goal_x == 1.25 and goal_y == -1.73:
                loc_name = "KITCHEN"
            else:
            ...
            current_state = SM_LEFT_MOVE
        elif current_state == SM_LEFT_MOVE:
            print("Moving the left arm close to the object")
            move_left_arm(-0.01,0,0,0.01,0,0,0)
            move_left_arm(-0.3,-0.01,0.01,1,0,0.07,-0.01)
            move_left_arm(0.25,-0.05,0,1.8,0,0.14,-0.07)
            move_left_arm(0.5,-0.12,0.05,1.5,0.1,0.2,-0.18)
            ...
            current_state = SM_HOLD_PRINGLES     
        elif current_state == SM_HOLD_PRINGLES:
            move_left_arm_with_trajectory(movement)
            print("MOVING ARM TO THE PRINGLES")
            time.sleep(2.0)
            print("Arm goal reached")
            current_state = SM_CLOSE_GRIP
        elif current_state == SM_CLOSE_GRIP:
            move_left_gripper(-1)
            print("GRIP CLOSED")

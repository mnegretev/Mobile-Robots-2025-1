...
def calculate_control(goal_x, goal_y, alpha, beta):
    # Ángulo de error entre el robot (0, 0) y el objetivo
...
def attraction_force(goal_x, goal_y, eta):
    ...
    # Calcular fuerza atractiva
    force_x = -eta * goal_x / distance_to_goal
    force_y = -eta * goal_y / distance_to_goal
...
def rejection_force(laser_readings, zeta, d0):
    ...
    for [distance, angle] in laser_readings:
        if distance < d0 and distance > 0:  # Solo consideramos los obstáculos dentro del rango d0
            # Calcular el factor de la fuerza repulsiva
            repulsion_factor = zeta * (1.0 / distance - 1.0 / d0) / (distance ** 2)
            
            # Convertir la fuerza repulsiva a componentes x e y
            force_x += repulsion_factor * math.cos(angle)
            force_y += repulsion_factor * math.sin(angle)
    
    # Normalizar las fuerzas repulsivas por el número de lecturas
    force_x /= N
    force_y /= N

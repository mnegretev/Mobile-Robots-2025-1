...
NAME = "SANTIAGO CALDERON TORRES"

def calculate_control(goal_x, goal_y):
    v,w = 0,0
    #
    # TODO:
    ...
    v = v_max * math.exp( - error_a * error_a / 0.2)
    w = w_max * (2 / (1 + math.exp( - error_a / 0.2)) - 1)   
...
def attraction_force(goal_x, goal_y, eta):
    ...
    #
    force_x = - eta * (goal_x / numpy.sqrt(goal_x ** 2 + goal_y ** 2))
    force_y = - eta * (goal_y / numpy.sqrt(goal_x ** 2 + goal_y ** 2))

...
#        cmd = cv2.waitKey(0)
    ...
    with open("/home/chover/analisis.csv", "w", newline="", encoding="utf-8") as file:
        micsv = csv.writer(file)
        micsv.writerow(["Aprendizaje", "Epocas", "Lote", "(ms)", "% acierto", "% error"])
        for learning_rate in [0.5, 1.0, 3.0, 10.0]:
            for epochs in [3, 10, 50, 100]:
                for batch_size in [5, 10, 30, 100]:
                    ...
                    aciertos="%"+str((hits/100)*100)
                    errores="%"+str((nohits/100)*100)
                    micsv.writerow([learning_rate, epochs, batch_size, tiempo, aciertos, errores])

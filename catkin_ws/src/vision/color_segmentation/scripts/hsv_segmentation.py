...
def segment_by_color(img_bgr, points, obj_name):
    ...
    lower_PD = (25, 50, 50) if obj_name == "pringles" else (15, 50, 50) 
    upper_PD = (35, 255, 255) if obj_name == "pringles" else (16, 255, 255)
    
    hsv = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2HSV)
    mask = cv2.inRange(hsv,lower_PD,upper_PD)
    test1 = mask
    
    kernelSize = 10
    kernel = cv2.getStructuringElement(cv2.MORPH_CROSS, (2*kernelSize+1, 2*kernelSize+1))
